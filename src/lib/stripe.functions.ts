...
export const createConnectOnboardingLink = createServerFn({ method: "POST" })
  ...
  .inputValidator((d) => z.object({ action: z.literal("connect") }).parse(d))
...
export const refreshStripeStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) => z.object({ action: z.literal("refresh") }).parse(d))
  .handler(async ({ context }) => {
    const { getStripe } = await import("./stripe.server");
    const stripe = getStripe();
    const { supabase, userId } = context;
...
export const testStripeConnection = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) => z.object({ action: z.literal("test") }).parse(d))
  .handler(async () => {
    const key = process.env.STRIPE_SECRET_KEY;
    if (!key) {
